"""
chat.py – Simple rule-based chat handler for Mind Bloom.

This module defines a FastAPI router that exposes a `/chat` endpoint for
conversational interactions.  It implements a lightweight, in‑memory
matching system using TF‑IDF to map user messages onto a set of
predefined prompts and responses.  No external APIs or paid services
are required.
...
# extend this dictionary with additional Q&A pairs.  When adding
# mental‑health related prompts, keep responses empathetic and avoid
...
FAQ: Dict[str, List[str]] = {
    ...
    "epds assessment results": [
        "Hello! I'm here to help you understand your EPDS assessment results. Based on your responses, you may have a high risk of postpartum depression. How can I assist you today?"
    ],
    "assessment results": [
        "Hello! I'm here to help you understand your EPDS assessment results. Based on your responses, you may have a high risk of postpartum depression. How can I assist you today?"
    ],
...
# Precompute the TF‑IDF vectors for the FAQ keys.  This allows us to
# perform cosine similarity on user input to find the best matching
# prompt.
...
def chat_endpoint(req: ChatRequest) -> ChatResponse:
    ...
    if key:
        responses = FAQ[key]
        reply = responses[0]  # deterministic; use random.choice() if you prefer variety
    else:
