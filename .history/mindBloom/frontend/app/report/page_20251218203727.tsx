      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        {loading ? (
          ...
          <>
            ...
            {/* Aggregate SHAP Feature Importance - XAI Insights */}
            <section className="mb-12 p-8 sm:p-10 rounded-3xl bg-gradient-to-br from-indigo-50/90 via-purple-50/80 to-violet-50/90 backdrop-blur-2xl border border-indigo-200/60 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-700">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-3xl font-bold text-slate-800">XAI - SHAP Feature Importance</h2>
                </div>
                {aggregateShap?.predictions_analyzed && (
                  <span className="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-sm font-semibold">
                    Based on {aggregateShap.predictions_analyzed} assessments
                  </span>
                )}
              </div>

              <p className="text-slate-700 mb-6 text-sm">
                Explainable AI (XAI) insights showing the most common factors influencing PPD risk predictions across all assessments. SHAP values reveal which factors contribute most to risk levels.
              </p>

              {shapLoading ? (
                <div className="p-8 text-center">
                  <div className="inline-block">
                    <div className="animate-spin rounded-full h-8 w-8 border-4 border-indigo-300 border-t-indigo-600"></div>
                    <p className="mt-2 text-slate-600 text-sm">Loading SHAP analysis...</p>
                  </div>
                </div>
              ) : aggregateShap?.aggregate_shap ? (
                <>
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                    {/* Top Risk Increasing Factors */}
                    <div className="p-6 bg-white/70 rounded-2xl border-2 border-red-200 shadow-lg">
                      ...
                      <div className="space-y-3">
                        {aggregateShap.aggregate_shap.increasing_risk.length > 0 ? (
                          aggregateShap.aggregate_shap.increasing_risk.map((feature, idx) => (
                            <div key={`agg-risk-inc-${idx}`} className="p-3 bg-red-50 rounded-lg border border-red-200">
                              ...
                              <p className="text-xs text-red-700">
                                Avg impact: <span className="font-semibold">{feature.avg_impact.toFixed(3)}</span>
                              </p>
                              <div className="w-full h-1.5 bg-red-200 rounded-full overflow-hidden mt-2">
                                <div
                                  className="h-full bg-gradient-to-r from-red-400 to-red-600"
                                  style={{ width: `${Math.min(feature.avg_impact * 100, 100)}%` }}
                                />
                              </div>
                            </div>
                          ))
                        ) : (
                          <p className="text-slate-500 text-sm italic">No risk-increasing factors found</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="p-6 bg-white/70 rounded-2xl border-2 border-green-200 shadow-lg">
                      ...
                      <div className="space-y-3">
                        {aggregateShap.aggregate_shap.decreasing_risk.length > 0 ? (
                          aggregateShap.aggregate_shap.decreasing_risk.map((feature, idx) => (
                            <div key={`agg-risk-dec-${idx}`} className="p-3 bg-green-50 rounded-lg border border-green-200">
                              <div className="flex items-center justify-between mb-1">
                          ...
                          <p className="text-slate-500 text-sm italic">No protective factors found</p>
                        )}
                      </div>
                    </div>
                  </div>

                  <div className="p-4 bg-indigo-50 border border-indigo-300 rounded-lg flex gap-3">
                    ...
                    <p className="text-xs text-indigo-700">
                      <span className="font-semibold">How to read:</span> These are the most frequently occurring factors across all assessments. The frequency count shows how often each factor appeared as significant. Higher avg impact means stronger influence on predictions.
                    </p>
                  </div>
                </>
              ) : (
                <div className="p-6 bg-amber-50 border-2 border-amber-200 rounded-2xl text-center">
                  <p className="text-amber-700 font-semibold">No SHAP data available yet</p>
                  <p className="text-amber-600 text-sm mt-1">Complete some assessments to see aggregate feature importance analysis</p>
                </div>
              )}
            </section>

            {/* Individual SHAP - Latest Assessment */}
            {latestShapExplanation && latestShapExplanation.top_features && (
              <section className="mb-12 p-8 sm:p-10 rounded-3xl bg-gradient-to-br from-violet-50/90 via-pink-50/80 to-rose-50/90 backdrop-blur-2xl border border-violet-200/60 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-700">
                <div className="flex items-center gap-3 mb-8">
                  <div className="w-10 h-10 rounded-lg bg-violet-100 flex items-center justify-center border border-violet-300">
                    <svg className="w-6 h-6 text-violet-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                  </div>
                  <div>
                    <h2 className="text-2xl font-bold text-slate-800">Your Latest Assessment</h2>
                    <p className="text-sm text-slate-600">Individual SHAP analysis from your most recent prediction</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
                  {/* Individual Risk Factors */}
                  <div className="p-6 bg-white/70 rounded-2xl border-2 border-violet-200 shadow-lg">
                    ...
                    <div className="space-y-3">
                      {latestShapExplanation.top_features
                        ...
                        .slice(0, 5)
                        .map((feature, idx) => (
                          <div key={`ind-risk-inc-${idx}`} className="p-3 bg-red-50 rounded-lg border border-red-200">
                            <div className="flex items-center justify-between mb-1">
                              <p className="font-semibold text-red-900 text-sm">{feature.feature.replace(/_/g, " ").toUpperCase()}</p>
                              <span className="text-xs font-bold text-red-600">{Math.abs(feature.shap_value).toFixed(3)}</span>
                            </div>
                            <p className="text-xs text-red-700">
                              Your value: <span className="font-semibold">{typeof feature.feature_value === "number" ? feature.feature_value.toFixed(2) : feature.feature_value}</span>
                            </p>
                          </div>
                        ))}
                    </div>
                  ...
                  <div className="p-6 bg-white/70 rounded-2xl border-2 border-green-200 shadow-lg">
                    ...
                    <div className="space-y-3">
                      {latestShapExplanation.top_features
                        ...
                        .slice(0, 5)
                        .map((feature, idx) => (
                          <div key={`ind-risk-dec-${idx}`} className="p-3 bg-green-50 rounded-lg border border-green-200">
                            <div className="flex items-center justify-between mb-1">
                              <p className="font-semibold text-green-900 text-sm">{feature.feature.replace(/_/g, " ").toUpperCase()}</p>
                ...
                {latestShapExplanation.risk_factors && (
                  <div className="p-6 bg-white/80 rounded-2xl border-2 border-purple-300">
                    <h3 className="font-bold text-purple-800 mb-4">📊 Your Assessment Summary</h3>
                        ...
                        <div>
                          ...
                          <ul className="space-y-1">
                            {latestShapExplanation.risk_factors.increasing_risk.slice(0, 5).map((factor, idx) => (
                              <li key={`ind-summary-inc-${idx}`} className="text-sm text-red-700 flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <ul className="space-y-1">
                            {latestShapExplanation.risk_factors.decreasing_risk.slice(0, 5).map((factor, idx) => (
                              <li key={`ind-summary-dec-${idx}`} className="text-sm text-green-700 flex items-center gap-2">
