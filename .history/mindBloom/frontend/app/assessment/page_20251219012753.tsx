...
export default function AssessmentPage() {
  ...
  // Check login and admin status on mount and storage changes
  useEffect(() => {
    const checkAuthStatus = () => {
      try {
        // Check mindbloom_user first (primary auth storage)
        const storedUser = localStorage.getItem("mindbloom_user");
        console.log("[Assessment] Checking auth, stored user:", storedUser);
        
        if (storedUser) {
          const parsed = JSON.parse(storedUser);
          console.log("[Assessment] Parsed user:", parsed);
          
          if (parsed.loggedIn) {
            setIsLoggedIn(true);
            setCurrentUser({ username: parsed.username, role: parsed.role });
            const adminStatus = parsed.role === "admin";
            console.log("[Assessment] Setting isAdmin to:", adminStatus);
            setIsAdmin(adminStatus);
            return; // Found valid user, no need for fallback
          }
        }
        
        // Fallback to "user" storage for compatibility
        const user = localStorage.getItem("user");
        if (user) {
          const parsed = JSON.parse(user);
          console.log("[Assessment] Fallback user:", parsed);
          const adminStatus = parsed.role === "admin";
          setIsAdmin(adminStatus);
        ...
        // No valid user found - reset states
        console.log("[Assessment] No user found, resetting auth state");
        setIsLoggedIn(false);
        setIsAdmin(false);
        setCurrentUser(null);
      } catch (e) {
        console.error("[Assessment] Auth check error:", e);
        setIsAdmin(false);
        setIsLoggedIn(false);
        setCurrentUser(null);
      }
    ...
    // Listen for storage changes (login/logout from other tabs or navbar)
    window.addEventListener("storage", checkAuthStatus);
    // Listen for custom auth change event (same-tab logout)
    window.addEventListener("authChange", checkAuthStatus);
    
    // Periodic check to catch any missed updates (like NavBar does)
  ...
  >
    ...
    {step === i && (
      ...
      {/* Admin Test Button - Bottom-right corner (visible only to admins) */}
      {isAdmin && (
        <div className="fixed bottom-6 right-6 z-50">
          <div className="relative">
            <button
              onClick={() => setShowTestMenu(!showTestMenu)}
              className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-110 flex items-center justify-center border-2 border-emerald-300"
              title="Admin Quick Test"
              aria-label="Toggle admin test menu"
            >
              <span className="text-lg">🧪</span>
            </button>
            
            {showTestMenu && (
              <div className="absolute bottom-12 right-0 p-3 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border-2 border-emerald-200 min-w-[160px] animate-in fade-in slide-in-from-bottom-2 duration-200">
                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-emerald-100">
                  <span>👑</span>
                  <p className="text-xs font-bold text-emerald-700 uppercase tracking-wider">Admin Test</p>
                </div>
                <div className="space-y-1">
                  <button
                    onClick={() => loadTestPreset("low")}
                    className="w-full text-left px-3 py-2 text-sm font-semibold rounded-xl hover:bg-green-50 text-green-700 transition-colors flex items-center gap-2 border border-transparent hover:border-green-200"
                  >
                    <span className="w-3 h-3 rounded-full bg-green-500"></span>
                    Low Risk
                  </button>
                  <button
                    onClick={() => loadTestPreset("medium")}
                    className="w-full text-left px-3 py-2 text-sm font-semibold rounded-xl hover:bg-yellow-50 text-yellow-700 transition-colors flex items-center gap-2 border border-transparent hover:border-yellow-200"
                  >
                    <span className="w-3 h-3 rounded-full bg-yellow-500"></span>
                    Medium Risk
                  </button>
                  <button
                    onClick={() => loadTestPreset("high")}
                    className="w-full text-left px-3 py-2 text-sm font-semibold rounded-xl hover:bg-red-50 text-red-700 transition-colors flex items-center gap-2 border border-transparent hover:border-red-200"
                  >
...
function Step5({ form, phq9Score, submitPrediction, loading, prediction, startNewPrediction, editInformation, isLoggedIn, currentUser }: any) {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="space-y-4 animate-in fade-in zoom-in-50 duration-700 delay-300">
        {/* Login Status Banner */}
        {!prediction && !isLoggedIn && (
          <div className="p-4 bg-amber-50 border-2 border-amber-300 rounded-2xl flex items-center gap-3">
            <span className="text-2xl">🔐</span>
            <div>
              <p className="font-bold text-amber-800">Login Required</p>
              <p className="text-sm text-amber-700">You need to login to get your risk assessment. Click the button below to continue.</p>
            </div>
          </div>
        )}
        
        {/* Logged In Status */}
        {!prediction && isLoggedIn && currentUser && (
          <div className="p-3 bg-green-50 border-2 border-green-300 rounded-2xl flex items-center gap-3">
            <span className="text-xl">✅</span>
            <p className="text-sm text-green-700">
              Logged in as <strong>{currentUser.username}</strong> - Ready to assess
            </p>
          </div>
        )}
        ...
        {!prediction && (
          <button
            ...
            disabled={loading}
            className={`w-full font-bold py-4 px-6 rounded-2xl hover:shadow-2xl active:scale-95 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed text-lg border-2 ${
              isLoggedIn 
                ? "border-purple-300 text-slate-800" 
                : "border-amber-400 text-amber-900"
            }`} 
            style={{
              backgroundImage: isLoggedIn 
                ? 'linear-gradient(180deg, #fce7f3 0%, #e9d5ff 50%, #bfdbfe 100%)'
                : 'linear-gradient(180deg, #fef3c7 0%, #fcd34d 50%, #f59e0b 100%)',
            }}
          >
            {loading ? "Analyzing..." : isLoggedIn ? "Get Risk Assessment" : "Login to Get Assessment →"}
          </button>
        )}
          ...
          {prediction && (
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              <button
                ...
                className="px-6 py-3 font-semibold rounded-2xl hover:shadow-xl transition-all duration-200 active:scale-95 text-white border-2 border-blue-400 bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 hover:from-blue-500 hover:via-purple-500 hover:to-pink-500"
              >
                💬 Chat with Assistant
              </button>
            </div>
