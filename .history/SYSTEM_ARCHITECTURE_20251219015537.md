...
### Model-Agnostic LLM System

The chatbot implements a **Strategy/Adapter pattern** supporting multiple AI providers with automatic fallback:
...
└─────────────────────────────────────────────────────────────────────────────────┘
                    ...
                    ┌───────────┴───────────┐
                    │                       │
          API Key Valid?              API Key Missing/Invalid?
                    │                       │
                    ▼                       ▼
        ┌───────────────────┐   ┌───────────────────────────┐
        │  External LLM     │   │  Fallback Chatbot         │
        │  (via Adapter)    │   │  (fallback_chatbot.py)    │
        └─────────┬─────────┘   └─────────────┬─────────────┘
                  │                           │
     ┌────────────┼────────────┐              │
     ▼            ▼            ▼              ▼
┌─────────┐ ┌─────────┐ ┌─────────┐   ┌─────────────────┐
│ OpenAI  │ │ Gemini  │ │ Mistral │   │ TF-IDF + FAQ    │
│  GPT-4  │ │  2.0    │ │  Large  │   │ Pattern Match   │
└─────────┘ └─────────┘ └─────────┘   │ (50+ responses) │
                                      └─────────────────┘
```
...
|----------|-----------------|---------------|-------|
| **OpenAI** | `OPENAI_API_KEY` | gpt-3.5-turbo | GPT-4, GPT-4-turbo available |
| **Google Gemini** | `GEMINI_API_KEY` | gemini-2.0-flash | gemini-1.5-pro available |
| **Mistral AI** | `MISTRAL_API_KEY` | mistral-large-latest | Direct Mistral API |
| **Anthropic** | `ANTHROPIC_API_KEY` | claude-3-sonnet | Claude 3 Opus/Haiku available |
| **OpenRouter** | `OPENROUTER_API_KEY` | mixtral-8x22b-instruct | Multi-provider gateway |

### Fallback Chatbot

When no API key is configured, the system uses a **rule-based fallback**:

- **Technology**: TF-IDF Vectorizer + Cosine Similarity
- **Responses**: 50+ predefined FAQ answers for PPD topics
- **Features**:
  - Crisis detection (suicide, self-harm keywords)
  - Risk-level specific guidance (low/medium/high)
  - PPD symptoms, treatment, resources
  - Self-care and coping strategies
- **Advantages**: Free, instant, works offline, medically curated

### Admin Configuration

Admins can configure LLM providers at runtime via:
- **UI**: `/admin/llm-settings` page
- **API**: `POST /admin/llm/configure`

Configuration is stored in server memory (not persisted to database for security).
